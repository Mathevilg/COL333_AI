...
// code from sir's board.cpp

bool protected_check(Board b, U8 piece_pos)
{
    Board b_copy = *b.copy();
...
int how_many_protected_score(Board& b)
{
    ...
    for (int i = 0; i < 64; i++)
    {
        ...
        {
            if (b.data.board_0[i] == PAWN) {
                if (protected_check(b, i))
                {
                    score += 3;
                }
...
int evaluate_function(Board b)
{
    ...
    int w3 = 2;
    int protected_score = how_many_protected_score(b);
    int w4 = 3;
    return (w1*material + w2*pawn_score + w3*check_score + w4*protected_score);
}
